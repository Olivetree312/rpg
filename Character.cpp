/*INFO - Generic Character --> PRIMARY STATS (0-100%)
 * PRIMARY STATS (0-100%)
...
//GETTERS AND SETTERS
	...
	* ~ basics: coin, bread, water, stick
	...
	void Character::updateSTATS(string primary){
		if (primary == "STR") {
            STATS["MP"] += STATS["MP"]/100 * STATS[primary];  
            STATS["AP"] += STATS["AP"]/100 * STATS[primary]; 
        }
        else if (primary == "DEX") {
            STATS["SPD"] += STATS["DEX"]/100 * STATS[primary];
            STATS["STH"] += STATS["DEX"]/100 * STATS[primary];
            STATS["DEF"] += STATS["DEX"]/100 * STATS[primary];
        }
        else if (primary == "CONS") {
            STATS["HP"] += STATS["CONS"]/100 * STATS[primary];
            STATS["STA"] += STATS["CONS"]/100 * STATS[primary];
        }
        else if (primary == "CHA") {
            STATS["AFF"] += STATS["CHA"]/100 * STATS[primary];
            STATS["DCP"] += STATS["CHA"]/100 * STATS[primary];
        }

        cout << "Updated secondary stats based on " << primary << "!" << endl;
	}
	
	void Character::setSKILL(string skill){
		SKILLS.push_back(skill);
		}
	void Character::setINVENTORY(string inventory, int num){
